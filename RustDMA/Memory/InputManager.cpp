...
bool c_keys::InitKeyboard()
{
	...
	this->win_logon_pid = TargetProcess.GetPidFromName("winlogon.exe");
	if (Winver > 22000)
	{
		...
		{
			auto pid = pids[i];
			uintptr_t tmp = VMMDLL_ProcessGetModuleBaseU(TargetProcess.vHandle, pid, const_cast<LPSTR>("win32ksgd.sys"));
	...
	else
	{
		...
		bool result = VMMDLL_Map_GetEATU(TargetProcess.vHandle, TargetProcess.GetPidFromName("winlogon.exe") | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, const_cast<LPSTR>("win32kbase.sys"), &eat_map);
		if (!result)
			return false;
		...
		if (gafAsyncKeyStateExport < 0x7FFFFFFFFFFF)
		{
			...
			WCHAR moduleName[MAX_PATH] = L"win32kbase.sys";
			auto result = VMMDLL_Map_GetModuleFromNameW(TargetProcess.vHandle, TargetProcess.GetPidFromName("winlogon.exe") | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, moduleName, &module_info, VMMDLL_MODULE_FLAG_NORMAL);
			if (!result)
			{
			...
			char str[32];
			if (!VMMDLL_PdbLoad(TargetProcess.vHandle, TargetProcess.GetPidFromName("winlogon.exe") | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, module_info->vaBase, str))
			{
			...
			if (!VMMDLL_PdbSymbolAddress(TargetProcess.vHandle, str, const_cast<LPSTR>("gafAsyncKeyState"), &gafAsyncKeyState))
			{
...
void c_keys::UpdateKeys()
{
	...
	VMMDLL_MemReadEx(TargetProcess.vHandle, this->win_logon_pid | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, gafAsyncKeyStateExport, reinterpret_cast<PBYTE>(&state_bitmap), 64, NULL, VMMDLL_FLAG_NOCACHE);
