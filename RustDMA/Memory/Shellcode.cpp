...
{
	int pid = TargetProcess.GetPidFromName(process_name);
	...
	if (!VMMDLL_ProcessGetInformation(TargetProcess.vHandle, pid, &process_info, &process_info_size))
	{
	...
	DWORD cSections = 0;
	if (!VMMDLL_ProcessGetSectionsU(TargetProcess.vHandle, pid, const_cast<LPSTR>(module.c_str()), NULL, 0, &cSections) || !cSections)
	{
	...
	if (!pSections || !VMMDLL_ProcessGetSectionsU(TargetProcess.vHandle, pid, const_cast<LPSTR>(module.c_str()), pSections, cSections, &cSections) || !cSections)
	{
	...
	for (int i = 0; i < cSections; i++)
	{
		...
		{
			codecave = VMMDLL_ProcessGetModuleBaseU(TargetProcess.vHandle, pid, const_cast<LPSTR>(module.c_str())) + ((pSections[i].VirtualAddress + pSections[i].Misc.VirtualSize)) + 0x10;
			if (!codecave)
				break;
		}
	}
	...
	if (!TargetProcess.Read(codecave, buffer.get(), function_size, pid))
	{
...
std::vector<uint64_t> c_shellcode::find_all_codecave(size_t function_size, const std::string& process_name)
{
	...
	std::vector<std::string> module_list = TargetProcess.GetModuleList(process_name);
...
bool c_shellcode::call_function(void* hook, void* function, const std::string& process_name)
{
	...
	*reinterpret_cast<uint64_t*>(jmp_bytes + 6) = reinterpret_cast<uintptr_t>(hook);
	...
	if (!TargetProcess.Read(reinterpret_cast<uintptr_t>(function), orig_bytes.get(), sizeof(jmp_bytes), pid))
		return 0;

	if (!VMMDLL_MemWrite(TargetProcess.vHandle, pid, reinterpret_cast<uintptr_t>(function), jmp_bytes, sizeof(jmp_bytes)))
	{
	...
	//Restore function
	if (!VMMDLL_MemWrite(TargetProcess.vHandle, pid, reinterpret_cast<uintptr_t>(function), orig_bytes.get(), sizeof(jmp_bytes)))
	{
